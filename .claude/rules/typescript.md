...
- No internal barrel files: don't add a folder `index.ts` to re-export siblings for shorter imports - import directly from the file. A single curated public-API entry per package is fine (e.g. `packages/shared`); prefer explicit named re-exports over `export *`.
